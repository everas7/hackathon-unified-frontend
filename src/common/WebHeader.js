...
const defaultProps = {
    ...
    goto: { label: "Set 'goto.label' prop", onPress: emptyFn },
    tabs: [{ label: "Set 'tab[idx].label' prop", onPress: emptyFn }],
};
    ...
    return (
        <>
            ...
            <View style={[styles.layout, styles.tabSection]} dataSet={{ media: ids.tabSection }}>
                {tabs.map(tab => (
                    <Text style={styles.tab} dataSet={{ media: ids.tab }} onPress={tab.onPress}>
                        {tab.label}
                    </Text>
                ))}
            </View>
